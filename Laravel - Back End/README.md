# Template Laravel API Project With Frontend VUE (TYPESCRIPT)
...
- [Running Development Server for Backend](#running-development-server-for-backend)
- [Running Development Server for Frontend](#running-development-server-for-frontend)
...
- Minimal version [PostgreSQL 14.x.x](https://www.postgresql.org/download/) - Primary database
- [Bruno](https://www.usebruno.com/) - API testing tool (recommended)
- Minimal version [Node JS v20.19.6 LTS](https://nodejs.org/en/download/package-manager)
- Minimal version [NPM v10.8.2](https://nodejs.org/en/download/package-manager)
- [Redis](https://redis.io/download) - Optional but recommended for caching and sessions development
...
1. Copy file `.env.example` to `.env` and configure database settings, payment gateway settings, and any other configurations you need.

```bash
cp .env.example .env
```

1. Generate application key using this command.

```bash
php artisan key:generate
```

1. Running migration and seeder for initial data.

```bash
php artisan migrate --seed
```

1. Running migration to initiate data for province, city, district, village for Indonesia.

```bash
php artisan laravolt:indonesia:seed
```

## Running Development Server for Backend
...
Once the server is running, access the Swagger API documentation at <http://localhost:8000/api/documentation>
<<<<<<< HEAD
=======

### Frontend Integration

This template currently has **no frontend build system**. You can:

1. **Consume the API from a separate frontend application** (React, Vue, Angular, mobile app, etc.)
2. **Add a frontend framework** to this project:
    ...
    # For Inertia.js (Laravel + Vue/React)
    composer require inertiajs/inertia-laravel
    ```

See [CLAUDE.md](CLAUDE.md) for more details on integrating a frontend.
