...
class AuthController extends Controller
{
    ...
    private function createAuthLogin(Request $request, User $user)
    {
        $token = $user->createToken(
            "auth_token",
            ["*"],
            Carbon::now()->addSeconds(config("sanctum.expiration") * 60),
        )->plainTextToken;
    ...
    public function login(Request $request)
    {
        ...
        return ResponseFormatter::success(
            $this->createAuthLogin($request, $user),
            "Login berhasil",
        );
    }
    ...
    public function me(Request $request)
    {
        ...
        return ResponseFormatter::success(
            ...
            "Data profile berhasil ditampilkan",
        );
    }
    ...
    public function changePassword(Request $request)
    {
        ...
        if (!Hash::check($request->current_password, $user->password)) {
            return ResponseFormatter::error(
                400,
                "Password sekarang yang Anda berikan salah",
            );
        }
    ...
    public function accountSecurityLevel(Request $request)
    {
        ...
        $levels = [
            1 => [
                ...
                "name" => "Weak",
                "description" =>
                    "Akun hanya menggunakan password, email belum diverifikasi, dan tidak ada proteksi tambahan.",
            ],
            2 => [
                "level" => 2,
                "name" => "Medium",
                "description" =>
                    "Email sudah diverifikasi, namun 2FA belum diaktifkan.",
            ],
            3 => [
                "level" => 3,
                "name" => "Strong",
                "description" =>
                    "Email sudah diverifikasi dan 2FA aktif (OTP via email atau Google Authenticator).",
            ],
        ];
    ...
    public function addEmail(Request $request)
    {
        ...
        return ResponseFormatter::success(
            null,
            "Email berhasil ditambahkan. Silakan cek email untuk verifikasi.",
        );
    }
    ...
    public function enableGoogle2fa(Request $request)
    {
        ...
        if ($user->is_google2fa_enabled) {
            return ResponseFormatter::error(
                400,
                "Akun sudah terhubung dengan Google Authenticator",
            );
        }
    ...
    public function qrcodeUrlGoogle2fa(Request $request)
    {
        ...
        if (!$user->is_google2fa_enabled) {
            return ResponseFormatter::error(
                400,
                "Akun tidak terhubung dengan Google Authenticator",
            );
        }
    ...
    public function method2fa(Request $request)
    {
        ...
        return ResponseFormatter::success(
            $data,
            "2FA metode verifikasi berhasil ditampilkan",
        );
    }
    ...
    public function verify2fa(Request $request)
    {
        ...
        switch ($request->input("type")) {
            case "email":
                ...
                if (
                    !$userOtp ||
                    Carbon::parse($userOtp->expired_at)->isPast() ||
                    $userOtp->used_at
                ) {
                    return ResponseFormatter::error(
                        400,
            ...
            case "google":
                try {
                    ...
                    if (!$valid) {
                        throw new \Error(
                            "Gagal verifikasi Google Authenticator",
                        );
                    }
